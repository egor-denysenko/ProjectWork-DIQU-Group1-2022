...
void clearBuffer(char *, unsigned char, unsigned char *);
...
void main(void) {
    ...
    while(1)
    {
        ...
        {
            decode();
            clearBuffer(dataReceived, 3, &indexRC);
            isReceived = 0;
        }
...
void UART_init(long int baudRate)
{
    ...
    TXSTA |= 0x24;
    RCSTA |= 0x90;
    SPBRG = (char) (8000000 / (unsigned long) (64UL * (unsigned long) baudRate)) - 1;
    INTCON = 0xC0;
    PIE1 = 0x20;
}
...
void decode(void)
{
    if((dataReceived[0] == '7') || (dataReceived[0] == '*'))
    {
        valore = (dataReceived[1] - '0') * 10 + (dataReceived[2] - '0');
# 126 "soluzioneMain.c"
    }
}
